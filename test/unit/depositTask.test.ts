...
describe("obtainedFromId exclusion", () => {
  beforeEach(() => {
    // @ts-ignore
  ...
  it("clears obtainedFromId and returns true when store is empty", () => {
    const creep = {
      memory: { obtainedFromId: "some-container" } as any,
      store: { getUsedCapacity: (): number => 0 },
      room: { storage: undefined },
      pos: { findClosestByRange: (): null => null, getRangeTo: (): number => 1 },
      transfer: (): number => 0,
      moveTo: (): number => 0
    };
  ...
  it("excludes storage whose id matches obtainedFromId and falls through to spawn", () => {
    const storageId = "storage-123";
    const storage = { id: storageId, store: { getFreeCapacity: (): number => 500 } };
    const spawn = { id: "spawn1", store: { getFreeCapacity: (): number => 100 } };
    let transferTarget: object | null = null;

    const creep = {
      memory: { obtainedFromId: storageId } as any,
      store: { getUsedCapacity: (): number => 50 },
      room: { storage },
      pos: {
        findClosestByRange: (constant: number): object | null => {
    ...
    assert.strictEqual(transferTarget, spawn, "should skip storage matching obtainedFromId and deposit into spawn");
  });

  it("clears obtainedFromId on successful completion (no targets left)", () => {
    const creep = {
      memory: { obtainedFromId: "container-abc" } as any,
      store: { getUsedCapacity: (): number => 50 },
      room: { storage: undefined },
      pos: {
        findClosestByRange: (): null => null,
